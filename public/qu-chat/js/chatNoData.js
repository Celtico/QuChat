...
socket.onmessage = function(evt){
    ...
    if(server.type == 'onMessage'){
        ...
        /*
         ...
         /// socketReceive  'type'=>'listUsers',  'usersCount' => count($clients),

         listUsers:

         OK - Load ajax listUsers
         OK - Set userCount
        ...
        /*
         ...
         /// socketReceive  'type'=>'onOpen', 'id_resource'=> $conn->resourceId, 'clients'=> $clients

        onOpen:

         - Send ajax (vars $id_user,$name,$id_resource,$clients)

         onOpenAjax: (controller)
        ...
        onLoadAjax:
         ...
         OK - Send message to by socket notify all users listUsers
         OK - Load ajax antique messages

         */

        /// socketSend  'type'=>'onOpen'

        var c='',client='',clients=server.clients,i,key;
        for(key in clients){
            client = clients[key];
            for(i in client){
                c += client[i]+',';
